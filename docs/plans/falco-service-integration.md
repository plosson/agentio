...
4. **Should `ProfilePlugin` gain a teardown hook?** It has `add`, `createClient` and `reauthenticate` (`src/plugins/types.ts:9-14`), but nothing that runs when a profile is *removed*. The host's generic remove path (`src/utils/profile-commands.ts:112`) deletes credentials without telling the service, so a removed falco profile leaves its refresh token live on Falco's servers until it expires — falcio revoked on logout. Falco now revokes on reauthentication, where the plugin controls the flow, but the removal half needs a contract addition. This affects every OAuth plugin equally, so it is a host decision rather than this port's.

5. **`ServiceName` and `Config.profiles` remain hand-maintained.** `docs/service-plugins.md` step 5 calls them legacy, "until those legacy types are derived from the registry". Falco adds the twentieth hand-written entry. Not this port's job, but it is one more argument for doing it.
