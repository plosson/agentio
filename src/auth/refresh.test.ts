...
const { getFreshCredentials, canRefresh, REFRESH_BUFFER_MS } = await import('./refresh');
...
beforeEach(async () => {
  ...
  await seedVault({
    config: { profiles: { jira: [{ name: 'stale' }, { name: 'fresh' }], revolut: [{ name: 'noexp' }], telegram: [{ name: 'bot' }] } },
    credentials: {
      jira: {
...
describe('getFreshCredentials', () => {
  test('a token inside the buffer is refreshed and written back with the rotated refresh token', async () => {
    const { credentials, refreshed } = await getFreshCredentials<{ accessToken: string; refreshToken: string }>('jira', 'stale');
  ...
  test('a fresh token is returned untouched', async () => {
    const { credentials, refreshed } = await getFreshCredentials<{ accessToken: string }>('jira', 'fresh');
  ...
  test('concurrent callers for one profile share a single refresh', async () => {
    const results = await Promise.all([
      getFreshCredentials<{ accessToken: string }>('jira', 'stale'),
      getFreshCredentials<{ accessToken: string }>('jira', 'stale'),
      getFreshCredentials<{ accessToken: string }>('jira', 'stale'),
    ]);
    expect(jiraRefresh).toHaveBeenCalledTimes(1);
  ...
  test('a missing expiry counts as stale for short-lived tokens', async () => {
    const { credentials, refreshed } = await getFreshCredentials<{ accessToken: string; expiryDate?: number }>('revolut', 'noexp');
    expect(refreshed).toBe(true);
  ...
  test('static credentials come back as stored', async () => {
    expect(canRefresh('telegram')).toBe(false);
