...
│   ├── key.ts               # API keys for remote agents: create/list/update/rotate/revoke
...
│   ├── keepalive.ts         # Periodic refresh so idle tokens do not expire of disuse
...
**Token keepalive** (`src/daemon/keepalive.ts`): a refresh token nothing uses dies of disuse, so the daemon walks every profile weekly and refreshes what has gone stale. **Unlocking starts the loop and locking stops it**, with a pass immediately on unlock, because a hub that boots locked would otherwise wait out a whole interval before its first pass. Each pass schedules the next from its own completion, so two can never overlap. It does not force: `getFreshCredentials` already picks the profiles whose access token has lapsed, which is every OAuth one and no static one. A profile with nothing stored is passed over rather than reported as a failure every pass, one broken profile does not abandon the rest, and the pass always resolves so a vault locked mid-walk logs `outcome=aborted` rather than an unhandled rejection. `AGENTIO_KEEPALIVE_HOURS` tunes the gap, clamped to 1-336 hours because an unclamped one overflows `setTimeout` and fires continuously; `0` turns it off. This prevents expiry from disuse only. An absolute lifetime and a revocation still need someone to reauthenticate on the hub host.
...
**Admin UI** (`src/daemon/ui/index.html`, one file with inline style and script, embedded into the binary with a `text` import): a centred Unlock view, then three tabs. Profiles shows the same data as `agentio status --json` as one row per profile sorted by name: name plus account or error on a second line, the service with its brand icon, the status badge with when it was last tested, a read-only switch, Test, and a ⋯ menu (copy reference, Delete profile and credentials). Chips filter by service or state client-side, combined with the name filter; a footer line carries the totals. API keys lists keys with their `agio1.…xxxx` hint, a Create panel, and a one-time token dialog. Settings holds the hub address and a danger zone with Lock. Destructive actions confirm in a `<dialog>`; results show as toasts. Details: `docs/design/remote-vault.md`, section Admin UI. Owner routes live under `/ui/api/*` behind an `httpOnly` session cookie that expires after 30 idle minutes; the vault itself does not re-lock. `POST /ui/api/unlock` is limited to 5 attempts a minute per address (`X-Forwarded-For` first, then the socket peer). Lock drops every session; `POST /ui/api/logout` (Sign out on Settings) drops only the caller's. Add and reauth still happen with the CLI on the hub host.
