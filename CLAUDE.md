...
The writes are `profile add`, `profile rename` and `profile remove`. Each runs as it does locally, with any OAuth or token dance on the agent, and reaches the hub as `PUT`, `PATCH` or `DELETE /v1/profiles/:service/:name`. They need a key with `canManageProfiles` (`agentio key create --can-manage-profiles`), and the gate refuses up front, before any prompt or browser, when the key lacks it. A profile the key's allow-list does not cover reads as absent for all three, so a scoped token learns nothing about the rest of the vault. `reauth` and the read-only toggle stay owner-only. Error mapping and the refused-command list: `docs/design/remote-vault.md`, section Remote mode in the CLI.
