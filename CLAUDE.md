...
**Token keepalive** (`src/daemon/keepalive.ts`): a refresh token nothing uses dies of disuse, so the daemon walks every profile once a day and refreshes what has gone stale. It does not force: `getFreshCredentials` already picks exactly the profiles whose access token has lapsed, which after a day is every OAuth one and no static one, and forcing would burn an Atlassian rotation for nothing. A locked vault is left alone, a profile with nothing stored is passed over rather than reported as a failure every pass, and one broken profile does not abandon the rest. `AGENTIO_REFRESH_HOURS` tunes the interval; `0` turns it off. This prevents expiry from disuse only. An absolute lifetime and a revocation still need someone to reauthenticate on the hub host.
