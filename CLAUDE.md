...
**Writes are atomic.** `updateVault(mutate)` in `src/vault/vault.ts` runs load-mutate-save under an in-process write lock, and `saveVault` joins the same queue. Anything that changes the vault from the daemon (refresh write-back, key touches, UI actions) must go through it or `updateConfig`; a bare load-then-save from two callers would drop one change. The path is resolved once per write and the cache is tied to the pointer it was loaded through. Under `bun test`, a vault write outside the OS temp directory throws; every test must run with `HOME` under a `mkdtemp` directory.
...
**Credential API** (`src/daemon/routes-v1.ts`): what remote agents call with `Authorization: Bearer agio1.…`. `GET /v1/profiles` lists what the key may use with the effective read-only flag (profile OR key). `GET /v1/profiles/:service/:name` reports `ok`, `needs_reauth`, or `no_creds`. `POST /v1/profiles/:service/:name/credentials` returns the credential object the local code expects, refreshed first with a 10-minute buffer, minus `refresh_token`, `refreshToken`, and `privateKey`. 401 bad token (5 failures a minute per address, then 429), 403 outside the allow-list, 404 unknown profile, 409 `TOKEN_EXPIRED` when a refresh is rejected, 503 locked. One audit line per credentials call on stdout; `lastUsedAt` is touched at most once a minute. Shared response helpers live in `src/daemon/http.ts`.
